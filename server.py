...
def capture_loop():
    ...
    print(f"[VIDEO] Stereo frame: {EYE_WIDTH}x{EYE_HEIGHT} + {EYE_WIDTH}x{EYE_HEIGHT}")
...
def update_mouse(yaw, pitch, roll):
    """Yaw -> X. For landscape iPhone orientation, use roll -> Y.

    The previous implementation used Euler pitch for vertical movement.
    With the phone held horizontally, the physical up/down head movement is
    represented by roll in the sensor stream. Using roll here makes vertical
    mouse movement work with the actual landscape VR orientation.
    """
    global last_yaw, last_vertical
    ...
    dy = 0 if abs(dvertical) < MOUSE_DEADZONE else round(-dvertical * MOUSE_SENSITIVITY)
    move_mouse(dx, dy)
...
def sensor_connect(phone_ip):
    ...
    while capture_running:
        ...
        try:
            ...
            print("[MOUSE] Yaw -> sağ/sol | Roll -> yukarı/aşağı")
            receive_sensor(sock)
            return
        except Exception as exc:
...
def parse_sensor(data):
    ...
    try:
        ...
        update_mouse(yaw, pitch, roll)
        print("\r" f"[SENSOR] Yaw: {yaw:+.3f}  " f"Pitch: {pitch:+.3f}  " f"Roll: {roll:+.3f}  " f"Q: {qx:+.2f},{qy:+.2f},{qz:+.2f},{qw:+.2f}", end="", flush=True)
    except Exception as exc:
...
def main():
    ...
    print("MOUSE AXES  : Yaw = sağ/sol, Roll = yukarı/aşağı")
    print()
